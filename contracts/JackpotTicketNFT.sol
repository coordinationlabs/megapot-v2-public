 ...
 */
contract JackpotTicketNFT is ERC721, Ownable2Step, IJackpotTicketNFT {
     ...
     * @param _ticketArt Address of the ticket art contract
     * @custom:effects
     ...
     */
    constructor(IJackpot _jackpot, ITicketArt _ticketArt) Ownable(msg.sender) {
        jackpot = _jackpot;
        ticketArt = _ticketArt;
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        if (tickets[tokenId].packedTicket == 0) revert TicketDoesNotExist();
        ExtendedTrackedTicket memory ticket = _getExtendedTicketInfo(tokenId);
        return ticketArt.generateTokenURI(ticket);
    }
     ...
     * @dev Sets the contract URI for the ticket NFT
    ...
    /**
     * @notice Sets the ticket art contract
     * @dev Sets the ticket art contract for the ticket NFT
     * @param _ticketArt The new ticket art contract
     * @custom:requirements
     * - Only owner can call
     * @custom:effects
     * - Sets the ticket art contract
     */
    function setTicketArt(ITicketArt _ticketArt) external onlyOwner {
        if (address(_ticketArt) == address(0)) revert JackpotErrors.ZeroAddress();
        ticketArt = _ticketArt;
        emit TicketArtUpdated(address(_ticketArt));
    }
    ...
    function _beforeTokenTransfer(address _from, address /* _to */, uint256 _tokenId) internal override {
        if (_from != address(0)) {
            ...
            if (fromTickets.totalTicketsBought == 0) revert NotTicketOwner();
