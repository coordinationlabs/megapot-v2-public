 ...
 */
contract ScaledEntropyProvider is Ownable2Step, AllowedManager, IScaledEntropyProvider, IEntropyConsumer {
    // =============================================================
    //                           STRUCTS
    // =============================================================
     ...
     */
    constructor(address _entropy, address _entropyProvider) AllowedManager() {
     ...
     * - For without replacement number of samples must not exceed range
     ...
     * @param _sequence Unique identifier of the entropy request
    ...
    function getPendingRequest(uint64 _sequence) external view returns (PendingRequest memory) {
        return pending[entropyProvider][_sequence];
    }
    ...
    function entropyCallback(uint64 sequence, address provider, bytes32 randomNumber) internal override {
        PendingRequest memory req = pending[provider][sequence];
        if (req.callback == address(0)) revert UnknownSequence();
        
        delete pending[provider][sequence];
    ...
    function _validateRequests(SetRequest[] memory _requests) internal pure {
        ...
        for (uint256 i = 0; i < _requests.length; i++) {
            ...
            if (_requests[i].samples == 0) revert InvalidSamplesCount();
            if (
                !_requests[i].withReplacement &&
                _requests[i].samples > _requests[i].maxRange - _requests[i].minRange + 1
            ) revert InvalidSamplesCount();
        }
    }
    ...
    function _storePendingRequest(
        ...
        pending[entropyProvider][_sequence].callback = msg.sender;
        pending[entropyProvider][_sequence].selector = _selector;
        pending[entropyProvider][_sequence].context = _context;
        for (uint256 i = 0; i < _setRequests.length; i++) {
            pending[entropyProvider][_sequence].setRequests.push(_setRequests[i]);
        }
    }
