...
contract Jackpot is IJackpot, Ownable2Step, ReentrancyGuardTransient {
    ...
    uint256 constant MAX_REFERRAL_FEE = 25e16; // 25%
    uint256 constant MAX_REFERRAL_WIN_SHARE = 25e16; // 25%
    uint256 constant MIN_LP_EDGE_TARGET = 5e16; // 5%
    uint256 constant MAX_LP_EDGE_TARGET = 5e17; // 50%
    ...
    mapping(uint256 => IJackpot.DrawingState) internal drawingState; // drawing => drawing state
    ...
    uint8 public bonusballSoftCap;
    uint8 public bonusballHardCap;
     ...
     * @param _bonusballSoftCap Maximum bonusball used when calculating LP pool cap (can be exceeded)
     * @param _bonusballHardCap Maximum bonusball used during drawing initialization (cannot be exceeded)
    ...
    constructor(
        ...
        uint8 _bonusballMin,
        uint8 _bonusballSoftCap,
        uint8 _bonusballHardCap,
        ...
        bonusballSoftCap = _bonusballSoftCap;
        bonusballHardCap = _bonusballHardCap;
        ...
        returns (uint256[] memory ticketIds)
    {
        ...
        IJackpot.DrawingState storage currentDrawingState = drawingState[currentDrawingId];
        
        uint256 numTicketsBought = _tickets.length;
        uint256 ticketsValue = numTicketsBought * currentDrawingState.ticketPrice;
        (uint256 referralFeeTotal, bytes32 referralSchemeId) = _validateAndTrackReferrals(
            currentDrawingState.referralFee,
            _referrers,
            _referralSplit,
            ticketsValue
        );
    ...
    function claimWinnings(uint256[] memory _userTicketIds) external nonReentrant {
        ...
        for (uint256 i = 0; i < _userTicketIds.length; i++) {
            ...
            uint256 winningAmount = winningDrawingState.payoutCalculator.getTierPayout(drawingId, tierId);
    ...
    function emergencyRefundTickets(uint256[] memory _userTicketIds) external nonReentrant onlyEmergencyMode {
        ...
        for (uint256 i = 0; i < _userTicketIds.length; i++) {
            ...
            DrawingState memory currentDrawingState = drawingState[currentDrawingId];
            uint256 refundAmount = ticketInfo.referralScheme == bytes32(0) ? currentDrawingState.ticketPrice 
                : currentDrawingState.ticketPrice * (PRECISE_UNIT - currentDrawingState.referralFee) / PRECISE_UNIT;
     ...
     * @custom:requirements
     * - Jackpot must have been initialized
        ...
        noEmergencyMode
    {
        ...
        emit JackpotSettled(currentDrawingId - 1, currentDrawingState.lpEarnings, drawingUserWinnings, _randomNumbers[1][0].toUint8(), winningNumbers, newAccumulatorValue);
    }
     ...
     * @param _protocolFeeAddress Address that receives protocol fees
    ...
    function initializeLPDeposits(uint256 _governancePoolCap) external onlyOwner {
        ...
        _validateAndUpdateLpPoolCap(normalBallMax);
    }
     ...
     * @custom:requirements
     * - Only owner can call
     * - Must not be less than 2 * NORMAL_BALL_COUNT in order to ensure there can be a no match tier
     * - Must not be greater than 128 since Combinations library constrains at that value
    ...
    function setNormalBallMax(uint8 _normalBallMax) external onlyOwner {
        _validateBallSettings(_normalBallMax, bonusballMin, bonusballSoftCap, bonusballHardCap);

        uint8 oldNormalBallMax = normalBallMax;
        _validateAndUpdateLpPoolCap(_normalBallMax);
        normalBallMax = _normalBallMax;
    ...
    function setBonusballMin(uint8 _bonusballMin) external onlyOwner {
        _validateBallSettings(normalBallMax, _bonusballMin, bonusballSoftCap, bonusballHardCap);
    ...
    /**
     * @notice Updates the bonusball soft cap for dynamic bonusball range adjustment
     * @dev The bonusball soft cap is the bonusball number used to calculate the lpPoolCap.
     * It acts as a buffer to ensure that deposits cannot be in the lpPool that force the next
     * drawing up to the hard cap. We consider the cap soft because bonusball ranges can still
     * exceed it but only from profits accrued from LPs. With proper parameterization this ensures
     * there is enough time for governance to raise the normal ball max to avoid hitting the bonusballHardCap.
     * @param _bonusballSoftCap New bonusball soft cap value
     * @custom:requirements
     * - Only owner can call
     * - Must satisfy: bonusballMin ≤ _bonusballSoftCap ≤ bonusballHardCap
     * - Ball settings validation ensures configuration consistency
     * @custom:effects
     * - Updates bonusballSoftCap state variable
     * - Recalculates LP pool capacity based on new configuration
     * @custom:emits BonusballSoftCapUpdated(currentDrawingId, oldValue, newValue)
     * @custom:security
     * - Validation ensures bonusball configuration remains mathematically sound
     * - LP pool capacity recalculation maintains economic security
     */
    function setBonusballSoftCap(uint8 _bonusballSoftCap) external onlyOwner {
        _validateBallSettings(normalBallMax, bonusballMin, _bonusballSoftCap, bonusballHardCap);

        uint8 oldBonusballSoftCap = bonusballSoftCap;
        bonusballSoftCap = _bonusballSoftCap;

        _validateAndUpdateLpPoolCap(normalBallMax);
        
        emit BonusballSoftCapUpdated(currentDrawingId, oldBonusballSoftCap, _bonusballSoftCap);
    }

    /**
     * @notice Updates the bonusball hard cap for maximum bonusball range
     * @dev The bonusball hard cap is the bonusball number that clamps the bonusball range. We
     * need a hard cap in order to avoid scenarios where the drawings cannot be settled due to
     * exceeding gas limits or over-packing the ticket bit vector. It is up to governance to properly
     * parameterize the hard cap to avoid these scenarios. Once the bonusballHardCap is hit, the bonusball
     * range can grow further from profits accrued from LPs but LPs will see deterioration in collected edge
     * per ticket.
     * @param _bonusballHardCap New bonusball hard cap value (must be ≥ bonusballSoftCap)
     * @custom:requirements
     * - Only owner can call
     * - Must satisfy: bonusballSoftCap ≤ _bonusballHardCap
     * - Must satisfy: bonusballHardCap + normalBallMax ≤ 255
     * @custom:effects
     * - Updates bonusballHardCap state variable
     * - Affects maximum possible bonusball range in future drawings
     * @custom:emits BonusballHardCapUpdated(currentDrawingId, oldValue, newValue)
     * @custom:security
     * - Validation prevents configurations that would break bonusball logic
     * - Hard cap prevents excessive bonusball expansion that could lock drawing settlement
     */
    function setBonusballHardCap(uint8 _bonusballHardCap) external onlyOwner {
        _validateBallSettings(normalBallMax, bonusballMin, bonusballSoftCap, _bonusballHardCap);

        uint8 oldBonusballHardCap = bonusballHardCap;
        bonusballHardCap = _bonusballHardCap;

        emit BonusballHardCapUpdated(currentDrawingId, oldBonusballHardCap, _bonusballHardCap);
    }
     ...
     * @custom:requirements
     * - Only owner can call
     * - Value must be greater than referralFee and less than PRECISE_UNIT
     * @custom:effects
    ...
    function setLpEdgeTarget(uint256 _lpEdgeTarget) external onlyOwner {
        if (_lpEdgeTarget < MIN_LP_EDGE_TARGET || _lpEdgeTarget > MAX_LP_EDGE_TARGET) revert JackpotErrors.InvalidLpEdgeTarget();
        if (_lpEdgeTarget <= referralFee) revert JackpotErrors.InvalidLpEdgeTarget();
     ...
     * @custom:requirements
     * - Only owner can call
     * - Value must not exceed lpEdgeTarget
     * @custom:effects
    ...
    function setReferralFee(uint256 _referralFee) external onlyOwner {
        if (_referralFee > MAX_REFERRAL_FEE) revert JackpotErrors.InvalidReferralFee();
        if (_referralFee >= lpEdgeTarget) revert JackpotErrors.InvalidReferralFee();
    ...
    function setReferralWinShare(uint256 _referralWinShare) external onlyOwner {
        if (_referralWinShare > MAX_REFERRAL_WIN_SHARE) revert JackpotErrors.InvalidReferralWinShare();
     ...
     * - Only owner can call
     * - Address must not be zero address
     * - Jackpot must not be locked (indicating draw is under way)
     * @custom:effects
    ...
    /**
     * @notice Returns tier payouts for a drawing using stored drawing parameters
     * @dev Convenience wrapper that automatically passes drawing state parameters to the payout calculator.
     *      For settled drawings, returns actual payouts. For current drawing, returns estimates based on
     *      theoretical ticket distribution (actual payouts depend on real ticket sales and duplicates).
     * @param _drawingId Drawing to query (must be valid drawing ID)
     * @return Array of payout amounts per winning ticket for all 12 tiers (in USDC wei)
     */
    function getDrawingTierPayouts(uint256 _drawingId) external view returns (uint256[12] memory) {
        if (_drawingId > currentDrawingId) revert JackpotErrors.InvalidDrawingId();
        DrawingState memory payoutsDrawingState = drawingState[_drawingId];
        return payoutsDrawingState.payoutCalculator.getExpectedDrawingTierPayouts(
            _drawingId,
            payoutsDrawingState.prizePool,
            payoutsDrawingState.ballMax,
            payoutsDrawingState.bonusballMax
        );
    }
    ...
    function _validateAndUpdateLpPoolCap(uint256 _normalBallMax) internal {
        uint256 lpSoftCap = _calculateLpPoolSoftCap(_normalBallMax);

        // We need to make sure that the lpPoolCap is not greater than the governance pool cap
        uint256 lpPoolCap = Math.min(lpSoftCap, governancePoolCap);
        jackpotLPManager.setLPPoolCap(currentDrawingId, lpSoftCap, lpPoolCap);
    }

    function _validateBallSettings(
        uint8 _normalBallMax,
        uint8 _bonusballMin,
        uint8 _bonusballSoftCap,
        uint8 _bonusballHardCap
        ...
        pure
    {
        if (_normalBallMax < 2 * NORMAL_BALL_COUNT || _normalBallMax > 128) revert JackpotErrors.InvalidNormalBallMax();
        if (_bonusballMin == 0) revert JackpotErrors.InvalidBonusballMin();
        if (_bonusballSoftCap <= _bonusballMin || _bonusballSoftCap >= _bonusballHardCap) revert JackpotErrors.InvalidBonusballSoftCap();
        if (_bonusballHardCap > MAX_BIT_VECTOR_SIZE - _normalBallMax) revert JackpotErrors.InvalidBonusballHardCap();
    }
    
    function _calculateLpPoolSoftCap(uint256 _normalBallMax) internal view returns (uint256) {
        // We use bonusballSoftCap in order to ensure that there is sufficient buffer before we hit the hard cap. Once the soft cap is hit
        // deposits cannot be made but the lpPool can continue to grow from profits with no adverse effects to LPs. Once the hard cap is hit
        // the lpPool can still grow from profits but LPs will see deterioration in collected edge per ticket.
        uint256 maxAllowableTickets = Combinations.choose(_normalBallMax, NORMAL_BALL_COUNT) * (bonusballSoftCap);
    ...
    function _setNewDrawingState(uint256 _newLpValue, uint256 _nextDrawingTime) internal {
        ...
        IJackpot.DrawingState storage newDrawingState = drawingState[currentDrawingId];
        ...
        newDrawingState.payoutCalculator = payoutCalculator;
        newDrawingState.referralFee = referralFee;
        ...
        // Calculate bonusball without capping it based on bonusballHardCap. We use uint256 here to avoid overflows.
        uint256 uncappedBonusball = Math.max(bonusballMin, Math.ceilDiv(minNumberTickets, combosPerBonusball));

        // Cap the bonusball based on bonusballHardCap - we use the hard cap here because we cannot let the bonusball go above
        // the hard cap otherwise we could lock the next drawing settlement due to exceeding gas limits or over-packing the ticket
        // bit vector. Note that we validate that the bonusballHardCap + normalBallMax is less than 255 in _validateBallSettings. It
        // is up to governance to set a hard cap that does not lead to exceeding gas limits.
        uint8 newBonusball;
        if (uncappedBonusball >= bonusballHardCap) {
            newBonusball = bonusballHardCap;
            emit BonusballHardCapHit(currentDrawingId, uncappedBonusball, bonusballHardCap);
        } else {
            // We are guaranteed that the bonusball will be less than 255 because we validate that bonusballHardCap + normalBallMax
            // is less than 255 in _validateBallSettings. Since uncappedBonusball is less than bonusballHardCap we know that the bonusball
            // will be less than 255.
            newBonusball = uint8(uncappedBonusball);
        }
        ...
        newDrawingState.payoutCalculator.setDrawingTierInfo(currentDrawingId);
        ...
        returns (uint256 referralFeeTotal, bytes32 referralSchemeId)
    {
        if (_referrers.length > 0) {
            ...
            referralFeeTotal = _ticketsValue * _referralFee / PRECISE_UNIT;
    ...
    function _validateAndStoreTickets(
        IJackpot.DrawingState storage _currentDrawingState,
        Ticket[] memory _tickets,
        address _recipient,
        ...
        returns(uint256 winningNumbers, uint256 drawingUserWinnings)
    {
        ...
        drawingUserWinnings = _currentDrawingState.payoutCalculator.calculateAndStoreDrawingUserWinnings(
            currentDrawingId,
        ...
        returns (uint256)
    {
        ...
        if (_referralSchemeId == bytes32(0)) {
            // If no LPs left or emergency mode is active just send referrer share to user since cannot accrue to LPs
            if (
                jackpotLPManager.getLPDrawingState(currentDrawingId).lpPoolTotal == 0 ||
                emergencyMode
            ) return 0;
