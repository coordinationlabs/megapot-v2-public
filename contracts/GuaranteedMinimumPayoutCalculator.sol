 ...
 */
contract GuaranteedMinimumPayoutCalculator is IPayoutCalculator, Ownable2Step {
     ...
     * - Premium tier 0 weight must be zero
     * - Min payout eligibility must be set to false for tier 0
    ...
    constructor(
        ...
        _setMinPayoutTiers(_minPayoutTiers);
    ...
    {   
        ...
        (
            ...
            uint256 minimumPayoutAllocation,
            bool useMinimumPayouts
        ) = _calculateTierWinnersAndPayoutStrategy(
            tierInfo,
            _prizePool,
            _normalMax,
            _bonusballMax,
        ...
        uint256 remainingPrizePool = useMinimumPayouts ? _prizePool - minimumPayoutAllocation : _prizePool;
        uint256[TOTAL_TIER_COUNT] memory drawingTierPayouts;
        (
            totalPayout,
            drawingTierPayouts
        ) = _calculateTierPayouts(
            tierInfo,
            remainingPrizePool,
        ...
        _storeTierPayouts(_drawingId, drawingTierPayouts);

        emit PrizeTierPayoutsCalculated(_drawingId, drawingTierPayouts);
    }
     ...
     * - Tier 0 must be set to 0 weight
     * @custom:emits None
     * @custom:effects
    ...
    function getDrawingTierPayouts(uint256 _drawingId) external view returns (uint256[TOTAL_TIER_COUNT] memory) {
        return _buildDrawingTierPayoutsArray(_drawingId);
    }

    /**
     * @notice Calculates expected tier payouts for a drawing (settled or unsettled)
     * @dev For settled drawings (past), returns the stored payout amounts that were calculated during settlement.
     *      For unsettled drawings (current), estimates theoretical payouts based on total possible winning combinations,
     *      assuming no user-owned duplicate tickets. The estimation uses the same payout calculation logic as settlement
     *      but with theoretical winner counts instead of actual ticket purchase data.
     *      
     *      Estimation logic:
     *      1) Calculates total possible winning tickets per tier (LP + theoretical user tickets)
     *      2) Determines if minimum payouts should be applied based on prize pool constraints
     *      3) Distributes prize pool using same premium tier weights and minimum payout logic as settlement
     *      
     *      Note: Estimates may differ from actual settlement due to:
     *      - User duplicate ticket purchases (not modeled in estimation)
     *      - Actual vs theoretical ticket sales distribution
     *      - Prize pool changes between estimation and settlement
     * 
     * @param _drawingId Drawing to calculate payouts for (must be <= current drawing ID)
     * @param _prizePool Total prize pool available for distribution (ignored for settled drawings)
     * @param _normalMax Maximum normal ball number for the drawing (must match drawing configuration)
     * @param _bonusballMax Maximum bonusball number for the drawing (must match drawing configuration)
     * @return drawingTierPayouts Array of payout amounts per winning ticket for all 12 tiers (in USDC wei)
     */
    function getExpectedDrawingTierPayouts(
        ...
        external
        view
        returns (uint256[TOTAL_TIER_COUNT] memory drawingTierPayouts)
    {
        DrawingTierInfo storage tierInfo = drawingTierInfo[_drawingId];
        if (_drawingId > jackpot.currentDrawingId()) revert InvalidDrawingId();
        if (_drawingId < jackpot.currentDrawingId()) return _buildDrawingTierPayoutsArray(_drawingId);
    ...
    function _setPremiumTierWeights(uint256[TOTAL_TIER_COUNT] memory _premiumTierWeights) internal {
        if (_premiumTierWeights[0] != 0) revert InvalidPremiumTierWeights();
        
        uint256 tierWeightSum = 0;
        for (uint256 i = 0; i < TOTAL_TIER_COUNT; i++) {
            tierWeightSum += _premiumTierWeights[i];
        }
        if (tierWeightSum != PRECISE_UNIT) revert InvalidPremiumTierWeights();

        premiumTierWeights = _premiumTierWeights;
    }

    function _setMinPayoutTiers(bool[TOTAL_TIER_COUNT] memory _minPayoutTiers) internal {
        if (_minPayoutTiers[0]) revert InvalidMinPayoutTiers();
        minPayoutTiers = _minPayoutTiers;
    }

    function _calculateTierWinnersAndPayoutStrategy(
        DrawingTierInfo storage _tierInfo,
        uint256 _prizePool,
        uint8 _normalMax,
        uint8 _bonusballMax,
        uint256[] memory _dupResult
        ...
        )
    {
        ...
        for (uint256 i = 0; i < TOTAL_TIER_COUNT; i++) {
            ...
            if (_tierInfo.minPayoutTiers[i]) {
                minimumPayoutAllocation += tierWinningTickets * _tierInfo.minPayout;
            }
        }
        
        // Only use minimum payouts if the premium tier minimum allocation + minimum payout allocation is less than the prize pool
        useMinimumPayouts = ((_prizePool * _tierInfo.premiumTierMinAllocation / PRECISE_UNIT) + minimumPayoutAllocation) < _prizePool;
    }

    function _calculateTierPayouts(
        DrawingTierInfo storage _tierInfo,
        ...
        internal
        view
        returns(uint256 totalPayout, uint256[TOTAL_TIER_COUNT] memory drawingTierPayouts)
    {
        for (uint256 i = 0; i < TOTAL_TIER_COUNT; i++) {
            // If no winners then no payout, this is inclusive of all LP-owned winning tickets
            if (_tierWinners[i] != 0) {
                ...
                uint256 premiumTierPayoutAmount = _remainingPrizePool * _tierInfo.premiumTierWeights[i] / (PRECISE_UNIT * _tierWinners[i]);
                // Add the premium tier payout to the minimum payout if the tier is eligible for the minimum payout
                uint256 tierPayout = _tierInfo.minPayoutTiers[i] ? _minPayout + premiumTierPayoutAmount : premiumTierPayoutAmount;
                // Add the payout for the tier to the array so it can be stored later and add the total tier payout to the total payout
                drawingTierPayouts[i] = tierPayout;
    ...
    function _storeTierPayouts(
        uint256 _drawingId,
        uint256[TOTAL_TIER_COUNT] memory _drawingTierPayouts
    )
        internal
    {
        for (uint256 i = 0; i < TOTAL_TIER_COUNT; i++) {
            tierPayouts[_drawingId][i] = _drawingTierPayouts[i];
        }
    }
    ...
    function _buildDrawingTierPayoutsArray(uint256 _drawingId) internal view returns(uint256[TOTAL_TIER_COUNT] memory) {
        uint256[TOTAL_TIER_COUNT] memory drawingTierPayouts;
        for (uint256 i = 0; i < TOTAL_TIER_COUNT; i++) {
