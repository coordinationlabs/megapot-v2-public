...
contract JackpotLPManager is IJackpotLPManager, Ownable {
    ...
    function processDeposit(uint256 _drawingId, address _lpAddress, uint256 _amount) external onlyJackpot() {
        // Note: this check also prevents deposits before initializeLP() since the pool cap will be 0.
        // Includes pending withdrawals converted from shares to USDC using the last settled accumulator,
        // yielding a conservative estimate of the next drawing LP pool for cap validation.
        LPDrawingState storage currentDrawingState = lpDrawingState[_drawingId];
        uint256 totalPoolValue = _calculateNextDrawingLpPool(_drawingId, currentDrawingState);
        ...
        currentDrawingState.pendingDeposits += _amount;

        emit LpDeposited(_lpAddress, _drawingId, _amount, currentDrawingState.pendingDeposits);
    }
     ...
     * @param _lpSoftCap Calculated maximum pool size based on the normal ball range, bonusball soft cap and other parameters
     ...
     */
    function setLPPoolCap(uint256 _drawingId, uint256 _lpSoftCap, uint256 _lpPoolCap) external onlyJackpot() {
        uint256 nextDrawingLpPool = _calculateNextDrawingLpPool(_drawingId, lpDrawingState[_drawingId]);

        // We need to ensure that soft cap is not exceeded by the next drawing's lp pool total regardless of
        // what the actual lpPoolCap is set to (since governance cap is more of a soft cap)
        if (_lpSoftCap <= nextDrawingLpPool) revert InvalidLPSoftCap();

        // In cases where governance is capping the pool size and remains unchanged we can skip the update
        // and checks. If governance cap is changed we need to ensure that the new cap is not exceeded by
        // the next drawing's lp pool total
        if (_lpPoolCap != lpPoolCap) {
            if (_lpPoolCap < nextDrawingLpPool) revert InvalidLPPoolCap();
            lpPoolCap = _lpPoolCap;

            emit LpPoolCapUpdated(_drawingId, _lpPoolCap);
        }
    }
    ...
    /**
     * @notice Returns the total active shares for an LP
     * @dev Calculates shares by combining consolidated shares with any unconsolidated deposits from previous drawings.
     *      Does not include pending deposits from the current drawing (those are still in USDC, not shares).
     * @param _lp Address of the LP to query
     * @return Total active shares for the LP
     */
    function getLPShares(address _lp) external view returns (uint256) {
        LP storage lp = lpInfo[_lp];
        uint256 currentDrawingId = jackpot.currentDrawingId();
        uint256 shares = lp.consolidatedShares;

        // Add unconsolidated deposits from previous drawings (converted to shares)
        if (lp.lastDeposit.drawingId < currentDrawingId && lp.lastDeposit.amount > 0) {
            shares += (lp.lastDeposit.amount * PRECISE_UNIT) / drawingAccumulator[lp.lastDeposit.drawingId];
        }
    ...
    /**
     * @notice Calculates an estimation for the LP pool value for the next drawing. Final value is impacted by outcome of the next drawing.
     * @dev Computes: lpPoolTotal + pendingDeposits - pendingWithdrawals (converted to USDC)
     *      This represents what the LP pool will be after the current drawing settles.
     * @return The projected LP pool value in USDC
     */
    function getEstimatedNextDrawingLpPool() external view returns (uint256) {
        uint256 currentDrawingId = jackpot.currentDrawingId();
        return _calculateNextDrawingLpPool(currentDrawingId, lpDrawingState[currentDrawingId]);
    }
    ...
    function _calculateNextDrawingLpPool(uint256 _drawingId, LPDrawingState storage _lp) internal view returns(uint256) {
        uint256 pendingWithdrawalsInUSDC = _drawingId == 0 ? 0 :
            _lp.pendingWithdrawals * drawingAccumulator[_drawingId - 1] / PRECISE_UNIT;
        return _lp.lpPoolTotal + _lp.pendingDeposits - pendingWithdrawalsInUSDC;
    }
}
