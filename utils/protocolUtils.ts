...
export const calculateLpPoolCap = (
    ...
    const maxAllowableTickets: bigint = BigInt(choose(Number(normalBallMax), 5) * Number(bonusballSoftCap));
...
export const calculateBonusballMax = (
    ...
    const uncappedBonusball = BigInt(Math.max(Number(bonusballMin), Math.ceil(Number(minNumberTickets) / Number(combosPerBonusball))));

    return BigInt(Math.min(Number(uncappedBonusball), Number(255) - Number(normalBallMax)));
}
...
export const generateClaimWinningsSignature = (
    ...
    const value = {
        ...
        bridgeDetails: {
            bridgeId: bridgeDetails.bridgeId,
            amount: bridgeDetails.amount
        }
    }
