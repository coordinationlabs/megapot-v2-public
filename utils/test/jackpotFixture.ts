...
export async function deployJackpotSystem(): Promise<JackpotSystemFixture> {
  ...
  const deploymentParams = {
    ...
    bonusballSoftCap: BigInt(100),
    bonusballHardCap: BigInt(150),
  ...
  const jackpot: Jackpot = await deployer.deployJackpot(
    ...
    deploymentParams.bonusballSoftCap,
    deploymentParams.bonusballHardCap,
  ...
  const jackpotNFT: JackpotTicketNFT = await deployer.deployJackpotTicketNFT(await jackpot.getAddress(), ADDRESS_ZERO);
