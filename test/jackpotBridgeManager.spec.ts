...
describe("JackpotBridgeManager", () => {
  ...
  describe("#constructor", async () => {
    it("should set the correct state variables", async () => {
      ...
      const actualRelayDepository = await jackpotBridgeManager.relayDepository();
      ...
      expect(actualRelayDepository).to.equal(await mockDepository.getAddress());
  ...
  describe("#claimWinnings", async () => {
    ...
    beforeEach(async () => {
      ...
      subjectBridgeDetails = {
        bridgeId: ethers.encodeBytes32String("test"),
        amount: expectedUserWinnings
      };
    ...
    it("should update the internal ticket ownership", async () => {
      await subject();
    ...
    describe("when the full amount of winnings is not bridged", async () => {
      beforeEach(async () => {
        subjectBridgeDetails.amount = expectedUserWinnings - usdc(1);
      ...
      it("should revert", async () => {
        await expect(subject()).to.be.revertedWithCustomError(jackpotBridgeManager, "InvalidBridgeAmount");
      });
    });
  ...
  describe("#getUserTickets", () => {
    ...
    describe("when user has tickets that have been transferred away", () => {
      ...
      it("should return array with zeros for transferred tickets", async () => {
        const userTickets = await subject();
        expect(userTickets.length).to.equal(0); // Array length stays same
      });
    ...
    describe("when user has mixed ownership (some transferred, some not)", () => {
      ...
      it("should return mixed array with zeros for transferred tickets", async () => {
        ...
        expect(userTickets[1]).to.equal(originalTicketIds[2]); // Still owned
      });
