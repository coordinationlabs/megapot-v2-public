...
describe("ScaledEntropyProvider", () => {
  ...
  describe("#requestAndCallbackScaledRandomness", async () => {
    ...
    beforeEach(async () => {
      await scaledEntropyProvider.connect(owner.wallet).addAllowed(user.address);
    ...
    describe("when a request has samples > maxRange - minRange + 1 but with replacement", async () => {
      beforeEach(async () => {
        subjectRequests = [
          {
            samples: 6,
      ...
      it("should store the pending request with correct parameters", async () => {
        ...
        expect(pendingRequest.setRequests[0].samples).to.eq(6);
        expect(pendingRequest.setRequests[0].minRange).to.eq(1);
        expect(pendingRequest.setRequests[0].maxRange).to.eq(5);
        expect(pendingRequest.setRequests[0].withReplacement).to.eq(true);
      });
    });
    ...
    describe("when a request has samples > maxRange - minRange + 1 but without replacement", async () => {
      beforeEach(async () => {
        subjectRequests = [
          {
            samples: 6,
      ...
      it("should revert with InvalidSamples error", async () => {
        await expect(subject()).to.be.revertedWithCustomError(scaledEntropyProvider, "InvalidSamplesCount");
      });
    });
    ...
    describe("when caller is not allowed", async () => {
      beforeEach(async () => {
        subjectCaller = attacker;
      });

      it("should revert with NotAllowed error", async () => {
        await expect(subject()).to.be.revertedWithCustomError(scaledEntropyProvider, "NotAllowed");
      });
    });
  });
  ...
  describe("#entropyCallback", async () => {
    ...
    beforeEach(async () => {
      ...
      await scaledEntropyProvider.connect(owner.wallet).addAllowed(await callbackMock.getAddress());
