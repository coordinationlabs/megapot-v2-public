...
describe("GuaranteedMinimumPayoutCalculator", () => {
  ...
  describe("#constructor", async () => {
    ...
    describe("when the premium tier weights do not sum to 1", async () => {
      beforeEach(async () => {
        subjectPremiumTierWeights = [
          ether(0),
          ether(0),
      ...
      it("should revert", async () => {
        await expect(subject()).to.be.revertedWithCustomError(payoutCalculator, "InvalidPremiumTierWeights");
      });
    });

    describe("when the tier 0 weight is not 0", async () => {
      beforeEach(async () => {
        subjectPremiumTierWeights = [
          ether(0.1),
          ether(0),
    ...
    describe("when the tier 0 min payout is not false", async () => {
      beforeEach(async () => {
        subjectMinPayoutTiers = [
          true,
          true,
      ...
      it("should revert", async () => {
        await expect(subject()).to.be.revertedWithCustomError(payoutCalculator, "InvalidMinPayoutTiers");
      });
    });
  ...
  describe("#calculateAndStoreDrawingUserWinnings", async () => {
    ...
    it("should emit the PrizeTierPayoutsCalculated event", async () => {
      const expectedTierPayouts = calculateTotalDrawingPayout(
        subjectPrizePool,
        subjectNormalMax,
        subjectBonusballMax,
      ...
      await expect(subject()).to.emit(payoutCalculator, "PrizeTierPayoutsCalculated").withArgs(
        subjectDrawingId,
        expectedTierPayouts.tierPayouts
      );
    });
  ...
  describe("#getDrawingTierPayouts", async () => {
    ...
    let normalMax: bigint;
    let bonusballMax: bigint;
    let result: bigint[];
    let dupResult: bigint[];

    beforeEach(async () => {
      subjectDrawingId = BigInt(1);
      await payoutCalculator.connect(mockJackpot.wallet).setDrawingTierInfo(subjectDrawingId);

      prizePool = usdc(2000000);
      normalMax = BigInt(35);
      bonusballMax = BigInt(9);
      result = [BigInt(1), BigInt(1), BigInt(1), BigInt(1), BigInt(1), BigInt(1), BigInt(1), BigInt(1), BigInt(1), BigInt(1), BigInt(1), BigInt(1)];
      dupResult = [BigInt(0), BigInt(0), BigInt(0), BigInt(0), BigInt(0), BigInt(0), BigInt(0), BigInt(0), BigInt(0), BigInt(0), BigInt(0), BigInt(0)];

      await payoutCalculator.connect(mockJackpot.wallet).calculateAndStoreDrawingUserWinnings(
        subjectDrawingId,
        prizePool,
        normalMax,
        bonusballMax,
    ...
    async function subject() {
      return await payoutCalculator.getDrawingTierPayouts(subjectDrawingId);
    }

    it("should return the correct tier payout", async () => {
      const actualTierPayouts = await subject();
      ...
      expect(actualTierPayouts).to.deep.equal(expectedTotalPayout.tierPayouts);
    });
  });

  describe("#getExpectedDrawingTierPayouts", async () => {
    let mockedPayoutCalculator: GuaranteedMinimumPayoutCalculator;
    ...
    beforeEach(async () => {
      const mockedJackpot = await deployer.deployMockJackpot();
      mockedPayoutCalculator = await deployer.deployGuaranteedMinimumPayoutCalculator(
        await mockedJackpot.getAddress(),
        minimumPayout,
        premiumTierMinAllocation,
        minPayoutTiers,
        premiumTierWeights
      ...
      await mockedJackpot.setDrawingTierInfo(
        mockedPayoutCalculator.getAddress(),
        BigInt(1)
      );

      const prizePool = usdc(2000000);
      const normalMax = BigInt(35);
      const bonusballMax = BigInt(9);
      const result = [BigInt(1), BigInt(1), BigInt(1), BigInt(1), BigInt(1), BigInt(1), BigInt(1), BigInt(1), BigInt(1), BigInt(1), BigInt(1), BigInt(1)];
      const dupResult = [BigInt(1), BigInt(100), BigInt(1), BigInt(100), BigInt(0), BigInt(0), BigInt(0), BigInt(0), BigInt(0), BigInt(0), BigInt(0), BigInt(0)];

      await mockedJackpot.connect(mockJackpot.wallet).calculateAndStoreDrawingUserWinnings(
        mockedPayoutCalculator.getAddress(),
        BigInt(1),
        prizePool,
        normalMax,
      ...
      await mockedJackpot.setDrawingId(BigInt(2));
      await mockedJackpot.connect(mockJackpot.wallet).setDrawingTierInfo(
        mockedPayoutCalculator.getAddress(),
        BigInt(2)
      );
    ...
    async function subject() {
      return await mockedPayoutCalculator.getExpectedDrawingTierPayouts(
        subjectDrawingId,
        subjectPrizePool,
        subjectNormalMax,
        subjectBonusballMax,
    ...
    it("should calculate and return the correct tier payouts", async () => {
      ...
      expect(tierPayouts).to.deep.equal(expectedTierPayouts.tierPayouts);
    });

    describe("when from current drawing and guaranteed minimum payout is greater than the prize pool", async () => {
      beforeEach(async () => {
        subjectDrawingId = BigInt(2);
        subjectPrizePool = usdc(500000);
      });

      it("should calculate and return the correct past drawing tier payouts", async () => {
        const tierPayouts = await subject();
    ...
    describe("when the drawing ID is from a past drawing", async () => {
      beforeEach(async () => {
        subjectDrawingId = BigInt(1);
      });
      ...
      it("should calculate and return the correct past drawing tier payouts", async () => {
        ...
        const expectedTierPayouts = calculateTotalDrawingPayout(
          ...
          subjectNormalMax,
          subjectBonusballMax,
          [BigInt(1), BigInt(1), BigInt(1), BigInt(1), BigInt(1), BigInt(1), BigInt(1), BigInt(1), BigInt(1), BigInt(1), BigInt(1), BigInt(1)],
          [BigInt(1), BigInt(100), BigInt(1), BigInt(100), BigInt(0), BigInt(0), BigInt(0), BigInt(0), BigInt(0), BigInt(0), BigInt(0), BigInt(0)],
          minimumPayout,
          minPayoutTiers,
          premiumTierWeights,
        );
    ...
    describe("when the drawing ID is greater than the current drawing ID", async () => {
      ...
      it("should calculate and return the correct past drawing tier payouts", async () => {
        await expect(subject()).to.be.revertedWithCustomError(mockedPayoutCalculator, "InvalidDrawingId");
      });
    });
  });
  ...
  describe("#setMinPayoutTiers", async () => {
    ...
    beforeEach(async () => {
      subjectMinPayoutTiers = [false, true, true, true, true, true, true, true, true, true, true, true];
      subjectCaller = owner;
    });
    ...
    describe("when the tier 0 is true", async () => {
      beforeEach(async () => {
        subjectMinPayoutTiers[0] = true;
      });
