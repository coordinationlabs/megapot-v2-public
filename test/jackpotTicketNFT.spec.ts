...
import { JackpotTicketNFT, MockJackpot, MockTicketArt } from "@utils/contracts";
...
describe("JackpotTicketNFT", () => {
  ...
  let mockTicketArt: MockTicketArt;
  ...
  describe("#constructor", () => {
    it("should set the jackpot and art address correctly", async () => {
      expect(await jackpotTicketNFT.jackpot()).to.equal(await mockJackpot.getAddress());
      expect(await jackpotTicketNFT.ticketArt()).to.equal(await mockTicketArt.getAddress());
    });

    it("should have correct name and symbol", async () => {
      expect(await jackpotTicketNFT.name()).to.equal("Megapot Tickets");
      expect(await jackpotTicketNFT.symbol()).to.equal("MEGAPOT");
    });
  });
  ...
  describe("Admin Functions", () => {
    describe("#setContractURI", () => {
      let subjectContractURI: string;
      let subjectCaller: Account;

      beforeEach(async () => {
        subjectContractURI = "https://example.com/contract-metadata";
        subjectCaller = owner;
      });

      async function subject(): Promise<any> {
        return await jackpotTicketNFT.connect(subjectCaller.wallet).setContractURI(subjectContractURI);
      }

      it("should set the contract URI", async () => {
        expect(await jackpotTicketNFT.contractURI()).to.equal("");

        await subject();

        expect(await jackpotTicketNFT.contractURI()).to.equal(subjectContractURI);
      });

      it("should emit ContractURIUpdated event", async () => {
        await expect(subject())
          .to.emit(jackpotTicketNFT, "ContractURIUpdated")
          .withArgs(subjectContractURI);
      });
      ...
      describe("when the caller is not the owner", () => {
        ...
        it("should revert", async () => {
          await expect(subject()).to.be.revertedWithCustomError(jackpotTicketNFT, "OwnableUnauthorizedAccount");
        });
      });
    });

    describe("#setTicketArt", () => {
      let subjectTicketArt: string;
      let subjectCaller: Account;

      beforeEach(async () => {
        subjectTicketArt = user1.address; // Use user1 as a mock ticket art address
        subjectCaller = owner;
      });

      async function subject(): Promise<any> {
        return await jackpotTicketNFT.connect(subjectCaller.wallet).setTicketArt(subjectTicketArt);
      }

      it("should set the ticket art address", async () => {
        expect(await jackpotTicketNFT.ticketArt()).to.equal(await mockTicketArt.getAddress());

        await subject();

        expect(await jackpotTicketNFT.ticketArt()).to.equal(subjectTicketArt);
      });

      it("should emit TicketArtUpdated event", async () => {
        await expect(subject())
          .to.emit(jackpotTicketNFT, "TicketArtUpdated")
          .withArgs(subjectTicketArt);
      });

      describe("when the ticket art address is zero address", () => {
        beforeEach(async () => {
          subjectTicketArt = ADDRESS_ZERO;
        });

        it("should revert", async () => {
          await expect(subject()).to.be.revertedWithCustomError(jackpotTicketNFT, "ZeroAddress");
        });
      });
  ...
  describe("#tokenURI", () => {
    ...
    describe("when ticket exists", () => {
      ...
      it("should return the URI from ticketArt", async () => {
        const result = await subject();
        expect(result).to.equal("mock://token-uri");
      });

      it("should return updated URI when mockTicketArt URI is changed", async () => {
        await mockTicketArt.setMockURI("https://new-uri.com/token");

        const result = await subject();
        expect(result).to.equal("https://new-uri.com/token");
      });
    });
    ...
    describe("when ticket does not exist", () => {
      ...
      it("should revert with TicketDoesNotExist", async () => {
        await expect(subject()).to.be.revertedWithCustomError(jackpotTicketNFT, "TicketDoesNotExist");
      });
    });
  });
  ...
  describe("Token transfers", () => {
    ...
    describe("when caller has no tickets in the drawing", () => {
      it("should revert with NotTicketOwner", async () => {
        // user2 has no tickets in DRAWING_ID_1 but tries to transfer user1's ticket
        await expect(
          jackpotTicketNFT.connect(user2.wallet).transferFrom(user2.address, user1.address, TICKET_ID_1)
        ).to.be.revertedWithCustomError(jackpotTicketNFT, "NotTicketOwner");
      });
    });
  });
});