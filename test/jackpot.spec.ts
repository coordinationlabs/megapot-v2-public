...
describe("Jackpot", () => {
  ...
  const bonusballHardCap: bigint = BigInt(130);
  ...
  describe("#constructor", async () => {
    it("should set the correct state variables", async () => {
      ...
      const actualBonusballSoftCap = await jackpot.bonusballSoftCap();
      const actualBonusballHardCap = await jackpot.bonusballHardCap();
      ...
      expect(actualBonusballSoftCap).to.eq(bonusballSoftCap);
      expect(actualBonusballHardCap).to.eq(bonusballHardCap);
  ...
  describe("#initialize", async () => {
    ...
    let subjectProtocolFeeAddress: Address;
    ...
    beforeEach(async () => {
      ...
      subjectProtocolFeeAddress = owner.address;
      subjectProtocolFee = protocolFee;
      subjectProtocolFeeThreshold = protocolFeeThreshold;
      subjectCaller = owner;
    });
    ...
    it("should set the correct state variables and initialize the contract", async () => {
      ...
      const postInitialized = await jackpot.initialized();
      ...
      const actualProtocolFee = await jackpot.protocolFee();
      const actualProtocolFeeThreshold = await jackpot.protocolFeeThreshold();

      expect(postInitialized).to.be.true;
      ...
      expect(actualProtocolFeeAddress).to.eq(subjectProtocolFeeAddress);
      expect(actualProtocolFee).to.eq(subjectProtocolFee);
      expect(actualProtocolFeeThreshold).to.eq(subjectProtocolFeeThreshold);
    });
    ...
    describe("runJackpot should not be callable by anyone before the contract is initialized", async () => {
      it("should revert", async () => {
        await expect(jackpot.runJackpot({ value: ether(1) })).to.be.revertedWithCustomError(jackpot, "JackpotNotInitialized");
      });
    });
  });
  ...
  context("when the contract has been initialized", async () => {
    ...
    context("when jackpot has been initialized", async () => {
      ...
      describe("#scaledEntropyCallback", async () => {
        ...
        let fillToSoftCap: boolean = false;
        ...
        beforeEach(async () => {
          ...
          if (fillToSoftCap) {
            const lpPoolCap = await jackpotLPManager.lpPoolCap();
            const currentLpDrawingState = await jackpotLPManager.getLPDrawingState(1);
            const depositAmount = lpPoolCap - currentLpDrawingState.lpPoolTotal;
            await usdcMock.connect(lpOne.wallet).approve(jackpot.getAddress(), depositAmount);
            await jackpot.connect(lpOne.wallet).lpDeposit(depositAmount);
          }
        ...
        describe("when the bonusballHardCap is hit", async () => {
          let newBonusballHardCap: bigint;
          ...
          beforeEach(async () => {
            newBonusballHardCap = bonusballSoftCap + BigInt(1);
            await jackpot.connect(owner.wallet).setBonusballHardCap(newBonusballHardCap);
            subjectRandomNumbers = [[BigInt(12), BigInt(13), BigInt(14), BigInt(15), BigInt(16)], [BigInt(17)]];
          });

          it("should set the bonusballMax to the bonusballHardCap", async () => {
            await subject();

            const currentDrawingState = await jackpot.getDrawingState(2);
            expect(currentDrawingState.bonusballMax).to.be.equal(newBonusballHardCap);
          });

          it("should emit the BonusballHardCapHit event with the correct values", async () => {
            const currentDrawingId = await jackpot.currentDrawingId();
            await expect(subject()).to.emit(jackpot, "BonusballHardCapHit").withArgs(currentDrawingId + BigInt(1), newBonusballHardCap, newBonusballHardCap);
          });
        });
        ...
        describe("when jackpot is in emergency mode", async () => {
          beforeEach(async () => {
            await jackpot.connect(owner.wallet).enableEmergencyMode();
          });
      ...
      describe("#claimWinnings", async () => {
        ...
        let withdrawAllLps: boolean = false;
        ...
        describe("when a passed ticket is not a winning ticket", async () => {
          ...
          it("should not accrue any referral fees", async () => {
            const preReferrerOneFees = await jackpot.referralFees(referrerOne.address);
            const preReferrerTwoFees = await jackpot.referralFees(referrerTwo.address);
            const preReferrerThreeFees = await jackpot.referralFees(referrerThree.address);
            ...
            expect(postReferrerOneFees).to.be.equal(preReferrerOneFees);
            expect(postReferrerTwoFees).to.be.equal(preReferrerTwoFees);
            expect(postReferrerThreeFees).to.be.equal(preReferrerThreeFees);
          });

          it("should emit TicketWinningsClaimed event with zero payout", async () => {
            // buyerOneTicketInfo[1] = { normals: [6, 7, 8, 9, 10], bonusball: 3 }
            // winningNumbers = [[1, 2, 3, 4, 5], [6]]
            // No matching normal balls, no matching bonusball
            await expect(subject()).to.emit(jackpot, "TicketWinningsClaimed").withArgs(
              subjectCaller.address,
              BigInt(1),            // drawingId
              subjectTicketIds[0],  // ticketId
              BigInt(0),            // matchCount (0 normal balls match)
              false,                // bonusballMatch
              BigInt(0)             // payout (zero for losing ticket)
            );
          });

          it("should emit ReferralFeeCollected events with zero amounts", async () => {
            // Events are still emitted for each referrer, but with 0 amounts
            await expect(subject())
              .to.emit(jackpot, "ReferralFeeCollected").withArgs(referrerOne.address, BigInt(0))
              .and.to.emit(jackpot, "ReferralFeeCollected").withArgs(referrerTwo.address, BigInt(0))
              .and.to.emit(jackpot, "ReferralFeeCollected").withArgs(referrerThree.address, BigInt(0));
          });
        ...
        describe("when a claimed ticket does not have a referral fee", async () => {
          ...
          it("should return the correct winnings (referral fee goes to LPs)", async () => {
            const preBuyerBalance = await usdcMock.balanceOf(buyerTwo.address);
            const preContractBalance = await usdcMock.balanceOf(await jackpot.getAddress());
            const preDrawingState = await jackpot.getDrawingState(2);
        ...
        describe("when a claimed ticket does not have a referral fee and there are no LPs left", async () => {
          before(async () => {
            withdrawAllLps = true;
          });

          after(async () => {
            withdrawAllLps = false;
          });
          ...
          it("should return the correct winnings (note referral fee goes to claimer)", async () => {
            const preBuyerBalance = await usdcMock.balanceOf(buyerTwo.address);
            const preContractBalance = await usdcMock.balanceOf(await jackpot.getAddress());
            const preDrawingState = await jackpot.getDrawingState(2);
            ...
            expect(actualWinningAmount).to.be.greaterThan(0);
            expect(actualPostBuyerBalance).to.be.equal(preBuyerBalance + actualWinningAmount);
            expect(actualPostContractBalance).to.be.equal(preContractBalance - actualWinningAmount);
            expect(actualPostDrawingState.lpEarnings).to.be.equal(preDrawingState.lpEarnings);
          });
        });

        describe("when a claimed ticket does not have a referral fee and emergency mode is active", async () => {
          beforeEach(async () => {
            await jackpot.connect(owner.wallet).enableEmergencyMode();
      ...
      describe("#getDrawingTierPayouts", async () => {
        ...
        beforeEach(async () => {
          ...
          subjectDrawingId = BigInt(2);
        });

        async function subject(): Promise<any> {
          return await jackpot.getDrawingTierPayouts(subjectDrawingId);
        }

        it("should claim the winnings", async () => {
          const tierPayouts = await subject();

          const drawingState = await jackpot.getDrawingState(subjectDrawingId);
          const winnings = calculateTotalDrawingPayout(
            drawingState.prizePool,
            drawingState.ballMax,
            drawingState.bonusballMax,
            Array(12).fill(BigInt(0)),
            Array(12).fill(BigInt(0)),
            minimumPayout,
            minPayoutTiers,
            premiumTierWeights,
          );
          expect(tierPayouts).to.deep.equal(winnings.tierPayouts);
        });

        describe("when the drawing ID is less than the current drawing ID", async () => {
          beforeEach(async () => {
            subjectDrawingId = BigInt(1);
          });

          it("should return the correct tier payouts", async () => {
            const tierPayouts = await subject();
            ...
            const winnings = calculateTotalDrawingPayout(
              ...
              drawingState.ballMax,
              drawingState.bonusballMax,
              // winners array doesn't matter because only duplicates have impact on payouts
              Array(12).fill(BigInt(0)),
              Array(12).fill(BigInt(0)),
              minimumPayout,
              minPayoutTiers,
        ...
        describe("when the drawing ID is greater than the current drawing ID", async () => {
          beforeEach(async () => {
            subjectDrawingId = BigInt(3);
          });

          it("should revert", async () => {
            await expect(subject()).to.be.revertedWithCustomError(payoutCalculator, "InvalidDrawingId");
          });
        });
      });
      ...
      describe("Admin Functions", async () => {
        ...
        describe("#setNormalBallMax", async () => {
          ...
          describe("when the normalBallMax is less than 2 * NORMAL_BALL_COUNT (5)", async () => {
            beforeEach(async () => {
              subjectNormalBallMax = BigInt(9);
            });

            it("should revert", async () => {
              await expect(subject()).to.be.revertedWithCustomError(jackpot, "InvalidNormalBallMax");
            });
          });


          describe("when the normalBallMax is greater than 128", async () => {
            beforeEach(async () => {
              subjectNormalBallMax = BigInt(130);
            });
          ...
          describe("when the normalBallMax + bonusballHardCap is greater than 255", async () => {
            beforeEach(async () => {
              subjectNormalBallMax = BigInt(127);
            });

            it("should revert", async () => {
              await expect(subject()).to.be.revertedWithCustomError(jackpot, "InvalidBonusballHardCap");
            });
          });
        ...
        describe("#setBonusballMin", async () => {
          ...
          describe("when the bonusballMin is greater than the bonusballSoftCap", async () => {
            beforeEach(async () => {
              subjectBonusballMin = BigInt(130);
            });

            it("should revert", async () => {
              await expect(subject()).to.be.revertedWithCustomError(jackpot, "InvalidBonusballSoftCap");
            });
          });
        ...
        describe("#setBonusballSoftCap", async () => {
          let subjectCaller: Account;
          let subjectBonusballSoftCap: bigint;

          beforeEach(async () => {
            subjectCaller = owner;
            subjectBonusballSoftCap = BigInt(105);
          });

          async function subject(): Promise<any> {
            return await jackpot.connect(subjectCaller.wallet).setBonusballSoftCap(subjectBonusballSoftCap);
          }

          it("should update the bonusballSoftCap", async () => {
            const preBonusballSoftCap = await jackpot.bonusballSoftCap();
            
            await subject();
            
            const postBonusballSoftCap = await jackpot.bonusballSoftCap();
            expect(postBonusballSoftCap).to.equal(subjectBonusballSoftCap);
            expect(postBonusballSoftCap).to.not.equal(preBonusballSoftCap);
          });

          it("should update the lpPoolCap on the LP manager", async () => {
            await subject();
            
            const expectedLpPoolCap = calculateLpPoolCap(normalBallMax, subjectBonusballSoftCap, ticketPrice, lpEdgeTarget, reserveRatio);
            const postLpPoolCap = await jackpotLPManager.lpPoolCap();
            expect(postLpPoolCap).to.equal(expectedLpPoolCap);
          });

          it("should emit BonusballSoftCapUpdated event", async () => {
            const preBonusballSoftCap = await jackpot.bonusballSoftCap();
            const currentDrawingId = await jackpot.currentDrawingId();
            
            await expect(subject())
              .to.emit(jackpot, "BonusballSoftCapUpdated")
              .withArgs(currentDrawingId, preBonusballSoftCap, subjectBonusballSoftCap);
          });

          describe("when the bonusballSoftCap is less than or equal to the bonusballMin", async () => {
            beforeEach(async () => {
              subjectBonusballSoftCap = BigInt(5);
            });
          ...
          describe("when the bonusballSoftCap is greater than the bonusballHardCap", async () => {
            beforeEach(async () => {
              subjectBonusballSoftCap = BigInt(140);
            });
        ...
        describe("#setBonusballHardCap", async () => {
          let subjectCaller: Account;
          let subjectBonusballHardCap: bigint;

          beforeEach(async () => {
            subjectCaller = owner;
            subjectBonusballHardCap = BigInt(135);
          });

          async function subject(): Promise<any> {
            return await jackpot.connect(subjectCaller.wallet).setBonusballHardCap(subjectBonusballHardCap);
          }

          it("should update the bonusballHardCap", async () => {
            const preBonusballHardCap = await jackpot.bonusballHardCap();
            
            await subject();
            
            const postBonusballHardCap = await jackpot.bonusballHardCap();
            expect(postBonusballHardCap).to.equal(subjectBonusballHardCap);
            expect(postBonusballHardCap).to.not.equal(preBonusballHardCap);
          });

          it("should emit BonusballHardCapUpdated event", async () => {
            const preBonusballHardCap = await jackpot.bonusballHardCap();
            const currentDrawingId = await jackpot.currentDrawingId();
            
            await expect(subject())
              .to.emit(jackpot, "BonusballHardCapUpdated")
              .withArgs(currentDrawingId, preBonusballHardCap, subjectBonusballHardCap);
          });

          describe("when the bonusballHardCap is less than the bonusballSoftCap", async () => {
            beforeEach(async () => {
              subjectBonusballHardCap = BigInt(100);
            });
          ...
          describe("when the bonusballHardCap + normalBallMax is greater than MAX_BIT_VECTOR_SIZE", async () => {
            beforeEach(async () => {
              subjectBonusballHardCap = BigInt(235);
            });
        ...
        describe("#setLpEdgeTarget", async () => {
          ...
          describe("when the lpEdgeTarget is < MIN_LP_EDGE_TARGET", async () => {
            beforeEach(async () => {
              subjectLpEdgeTarget = ether(0.01); // 1%
            });
          ...
          describe("when the lpEdgeTarget is >= MAX_LP_EDGE_TARGET", async () => {
            beforeEach(async () => {
              subjectLpEdgeTarget = ether(0.51); // 51%
            });
          ...
          describe("when the lpEdgeTarget is <= referralFee", async () => {
            beforeEach(async () => {
              subjectLpEdgeTarget = ether(0.065); // 6.5% to match referral fee
            });
        ...
        describe("#setReferralFee", async () => {
          ...
          describe("when the referralFee is >= MAX_REFERRAL_FEE", async () => {
            beforeEach(async () => {
              subjectReferralFee = ether(0.26); // 26%
            });
          ...
          describe("when the referralFee is >= lpEdgeTarget", async () => {
            beforeEach(async () => {
              await jackpot.connect(owner.wallet).setLpEdgeTarget(ether(0.2));
              subjectReferralFee = ether(0.21); // 21%
            });
        ...
        describe("#setReferralWinShare", async () => {
          ...
          describe("when the referralWinShare is > MAX_REFERRAL_WIN_SHARE", async () => {
            beforeEach(async () => {
              subjectReferralWinShare = ether(0.26); // 26%
            });
        ...
        describe("#setTicketPrice", async () => {
          ...
          it("should update the LP pool cap correctly", async () => {
            ...
            const expectedLPPoolCap = calculateLpPoolCap(normalBallMax, bonusballSoftCap, subjectTicketPrice, lpEdgeTarget, reserveRatio);
